...
if(ID==null||ID.equals("")){
	...
	<%	}else if(file.isDirectory()){ %>
		...
		File[] fileListinto = intodir.listFiles();
			for(int j=0; j<fileListinto.length; j++){
				...
				if(file2.isFile()){
			%>
				<li onclick="location=location='getTest.jsp?ID=<%=file.getName()%>&file=<%=file2.toString()%>&filename=<%=file2.getName()%>'"><%=file2.getName() %></li>
			<%
				}
			}
	...
	source = application.getRealPath("/WEB-INF/notebook/"+ID);
	...
	File[] fileList = dir.listFiles(); 
		for(int i = 0 ; i < fileList.length ; i++){System.out.println("!!!");
			...
			if(file.isFile()){ 
				// 파일이 있다면 파일 이름 출력
			%>
		<li onclick="location='getTest.jsp?file=<%=file.toString()%>&filename=<%=file.getName()%>'"><%=file.getName() %></li>			
	<%	}
	}
}
//out.println(source);
